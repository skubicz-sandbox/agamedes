...
public class MyCheckboxTreeHelper {

    private static final Key<Runnable> TREE_LISTENERS_REMOVER = Key.create("TREE_LISTENERS_REMOVER");
    public static final MyCheckboxTreeBase.CheckPolicy DEFAULT_POLICY = new MyCheckboxTreeBase.CheckPolicy(true, true, false, true);
    private final MyCheckboxTreeBase.CheckPolicy myCheckPolicy;
    private final EventDispatcher<CheckboxTreeListener> myEventDispatcher;

    public MyCheckboxTreeHelper(@NotNull MyCheckboxTreeBase.CheckPolicy checkPolicy, @NotNull EventDispatcher<CheckboxTreeListener> dispatcher) {
        myCheckPolicy = checkPolicy;
        myEventDispatcher = dispatcher;
    }

    public void initTree(@NotNull final Tree tree, JComponent mainComponent, MyCheckboxTreeBase.CheckboxTreeCellRendererBase cellRenderer) {
    ...
    public void setNodeState(Tree tree, CheckedTreeNode node, boolean checked) {
        System.out.println("setNodeState");
    ...
    private void adjustParentsAndChildren(final CheckedTreeNode node, final boolean checked) {
        if (!checked) {
            ...
            while (parent != null) {
                if (parent instanceof CheckedTreeNode) {
                    if(isChecked((CheckedTreeNode) parent)) {
                        changeNodeState((CheckedTreeNode) parent, true);
                    }
...
//            }
    ...
    private ClickListener setupMouseListener(final Tree tree, JComponent mainComponent, final MyCheckboxTreeBase.CheckboxTreeCellRendererBase cellRenderer) {
