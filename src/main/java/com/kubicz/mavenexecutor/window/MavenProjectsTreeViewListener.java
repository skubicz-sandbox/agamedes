...
public class MavenProjectsTreeViewListener extends MavenExecutorWindowListenerAdapter {
    ...
    private ProjectToBuild toProjectToBuild(Map.Entry<ProjectRootNode, List<Mavenize>> selectedProjectEntry) {
        ...
        if(projectRootNode.isSelected()) {
            projectToBuild = new ProjectToBuild(projectRootNode.getDisplayName(), projectRootNode.getMavenId(), projectRootNode.getProjectDirectory().getPath());
        }
        else {
            List<MavenId> modules = selectedModule.stream().map(Mavenize::getMavenId).collect(Collectors.toList());
            projectToBuild = new ProjectToBuild(projectRootNode.getDisplayName(), projectRootNode.getMavenId(), projectRootNode.getProjectDirectory().getPath(), modules);
        }
